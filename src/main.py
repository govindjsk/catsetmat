...
def perform_experiment(emb_args, home_path, data_path, data_name, split_id, result_path):
    ...
    num_epoch = 200
    train_auc = 0
    t = tqdm(range(num_epoch), 'AUC')
    for i in t:
        train_data1 = shuffle(train_data)
        train_loss_, train_auc = train(model, train_data1)
        test_loss_, test_auc = test(model, test_data)
...
def main():
    ...
    for split_id in tqdm(splits, 'Split #'):
        perform_experiment(emb_args, home_path, data_path, data_name, split_id, result_path)
